...
//divide a string em um objeto com multiplas questões
const createQuestionObject = (questionString,number) => {
  const regexString = '(.*)(?=\\(A\\))|(?<=\\(A\\))(.*)(?=\\(B\\))|(?<=\\(B\\))(.*)(?=\\(C\\))|(?<=\\(C\\))(.*)(?=\\(D\\))|(?<=\\(D\\))(.*)(?=\\(E\\))|(?<=\\(E\\))(.*)';
...
const extractQuestions = (text,start)=> {
  const end = start + 1;
  ...
  try {
    ...
    console.log(chalk.red(`Error: there is something wrong with question ${start}!`));
  }
}
...
export default async function catchFile(filePath,initalNumber,finalNumber)
{
  ...
  try {
    ...
    console.log(chalk.green(`Arquivo encontrado: ok!`));
  } catch (error){
    cureErrorFile(error);
  }
